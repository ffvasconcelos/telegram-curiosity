...
def working_process_bits(logger,slices,path,STORE_PATH,version):
    ...
    for fname in slices:
        # try:
            ...
            dframe = pd.read_csv(path+'/stimulus-filter-1-10-social-entropy-30-v8/'+\
            ...
            if dframe[dframe.user.isin(users_idx)].shape[0] >= 30:
                ...
                dframe['Tau_max']=np.vectorize(tau_largest_count_to_surprisal_compute)(dframe['ProbU'],dframe['Tau'])# getting the tau of max ProbU
                ...
                for fname2 in cols1:
                    ...
                    if not os.path.exists(STORE_PATH+'/stats-'+str(version)+'/'+fname2+'-group.tsv'):
                        dt.to_csv(STORE_PATH+'/stats-'+str(version)+'/'+fname2+'-group.tsv',sep='\t',header=True,float_format='%.2f',index=False)
                    else:
                    ...
                    if records % RECORDS_PERIOD == 0:
                        ...
                        for fname2 in cols1:
                            ...
                            if not os.path.exists(STORE_PATH+'/stats-'+str(version)+'/'+fname2+'.tsv'):
                                dt.to_csv(STORE_PATH+'/stats-'+str(version)+'/'+fname2+'.tsv',sep='\t',header=True,float_format='%.2f',index=False)
                            else:
                        ...
                        for fname2 in cols1:
                            ...
                            if not os.path.exists(STORE_PATH+'/stats-'+str(version)+'/'+fname2+'-raw.tsv'):
                                dt.to_csv(STORE_PATH+'/stats-'+str(version)+'/'+fname2+'-raw.tsv',sep='\t',header=True,float_format='%.2f',index=False)
                            else:
...
#                break
    ...
    if not os.path.exists(STORE_PATH+'/stats-'+str(version)+'/data_prob_cond.tsv'):
        dp.to_csv(STORE_PATH+'/stats-'+str(version)+'/data_prob_cond.tsv',index=False,header=False,sep='\t',float_format='%.2f')
    else:
    ...
    if not os.path.exists(STORE_PATH+'/stats-'+str(version)+'/data_tau.tsv'):
        dtau.to_csv(STORE_PATH+'/stats-'+str(version)+'/data_tau.tsv',index=False,header=False,sep='\t',float_format='%.2f')
    else:
    ...
    if ldata[0]!=[]:
        for fname in cols1:
            ...
            if not os.path.exists(STORE_PATH+'/stats-'+str(version)+'/'+fname+'.tsv'):
                dt.to_csv(STORE_PATH+'/stats-'+str(version)+'/'+fname+'.tsv',sep='\t',header=False,float_format='%.2f',index=False)
            else:
    ...
    if ldata2[0]!=[]:
        for fname in cols1:
            ...
            if not os.path.exists(STORE_PATH+'/stats-'+str(version)+'/'+fname+'-raw.tsv'):
                dt.to_csv(STORE_PATH+'/stats-'+str(version)+'/'+fname+'-raw.tsv',sep='\t',header=False,float_format='%.2f',index=False)
            else:
...
if __name__ == '__main__':   
    ...
    STORE_PATH='stimulus-bits-social-entropy-30-v8/'
