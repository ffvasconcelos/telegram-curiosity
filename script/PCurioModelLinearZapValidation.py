...
def matrix_to_dict_json(mtx_prob, dkey):
    dict_mtx_prob={"total": np.int(mtx_prob.sum()-np.diag(mtx_prob).sum())}
        ...
        for col in range(mtx_prob.shape[1]):
            if row != col and mtx_prob[row][col] != 0:
                ...
                dict_mtx_prob[str(user_number_row)][str(user_number_col)]['num']=\
                                                     np.int(mtx_prob[row][col])
        # ---------------------------------------------------------------------        
    return dict_mtx_prob


def computeStimulusDegree(fin,fout, group_id,window,path_mtx_prob,fname):
    if not os.path.exists("/media/alexandre/kingston/Whatsapp 2023/logs/"):
        os.makedirs("/media/alexandre/kingston/Whatsapp 2023/logs/")
    logger = Logger("/media/alexandre/kingston/Whatsapp 2023/logs/")
    logger.add_log({'empty':'key.empty'})
    fout.write("#user\tA\tB\tC\tD\tE\tF\tG\tGu\tH\tI\tL\tM\tN\tJ\tO\tP\tMu\tOu\tPu\tNu\tProbTl\tPindK\tPindU\tProbU\tTauU\tPO\tHd_partial\tHcondO\tHCONDO\tHD\tTotalU\tUs\tTotal\ttimestamp\n")
...
#                time_seconds=Time.mktime(time_date.timetuple())
            ...
            if size >= LOWER_BOUND_EVENTS:# storing the records in the file!!!
                ...
                ***************************************************************
                ...
                store_line+=','.join(('%d'%(pc)) for pc in ProbU)+'\t' # conditional probability of user to post after any other user u
                ...
                store_line+=','.join(('%.2f'%(hcond_o)) for hcond_o in HcondO)+'\t' # expectation entropy conditioned on origin
...
if __name__ == '__main__':
    ...
    PATH_FILTER=path+'groups-validation-model-1/'
    users_done=glob.glob(PATH_FILTER+'*')    
    lutodo=[]
    for u in users_done:
        path_name=u[len(PATH_FILTER):]
        fname=path_name.split('-')
        fname=fname[1]+'-'+fname[2]
        lutodo.append(path_name+'/'+fname+'.tsv')
    
    
    PATH_FILTER=path+"stimulus-filter-validation-model-1/"
    users_done=glob.glob(PATH_FILTER+'*')    
    ludone=[]
    for u in users_done:
        path_name=u[len(PATH_FILTER):]
        if len(path_name) > len('group_mtx_prob'):
            fname=path_name.split('-')
            fname=fname[1]+'-'+fname[2]
            ludone.append(path_name+'/'+fname+'.tsv')
    
    set_idx=set(lutodo).difference(ludone)
    
    groups_idx=np.array(list(set_idx))
    ...
    window=1
    start=Time.time()
    # working_process(path,['5521965830650-152851170'],window)
