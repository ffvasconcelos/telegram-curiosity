...
def update_contingency_table(queue_user_all, queue_time_all, dict_count, dict_mtx_prob, Dt, list_percentiles,
    ...
    for index in range(len(queue_user_all)):
        ...
        for key in dict_mtx_prob.keys():
            ...
            if (key != 'total') and (time - dict_mtx_prob[key]['time'] <= Dt):
                ...
                dict_mtx_prob[key][u]['tau'] += float(tau_array.sum())
        ...
        try:
            ...
            ************************************************
            ...
            if size >= LOWER_BOUND_EVENTS:  # storing the records in the file!!!
                ...
                store_line += ','.join(
                    ('%d' % (pc)) for pc in
                    ProbU) + '\t'  # conditional probability of user to post after any other user u
                store_line += ','.join(
