...
def computeStimulusDegree(fin, fout, group_id, window, path_mtx_prob, list_percentiles, percentiles, fname):
  if not os.path.exists(os.getcwd() + "/logs/%d-10-social-entropy-30-v6/" % window):
    os.makedirs(os.getcwd() + "/logs/%d-10-social-entropy-30-v6/" % window)
  logger = Logger(os.getcwd() + "/logs/%d-10-social-entropy-30-v6/" % (window))
  ...
  for idx in fin.index:
    ...
    *********** Media's Categorie ******************
    """
    message = fin['message'][idx]  # --------> 0
    urls = fin['urls'][idx]  # --------> 1
    imageID = fin['imageID'][idx]  # --------> 2
    videoID = fin['videoID'][idx]  # --------> 3
    audioID = fin['audioID'][idx]  # --------> 4
    # getting the vector of medias from user
    url_number = 0
    user_media, url_number = get_user_genres(message, urls, imageID, videoID, \
                                             audioID)
  ...
  with open(path_mtx_prob + group_id + '.txt', 'w') as json_file:
...
def working_process(path, slices, window):
  ...
  for fname in slices:
    print('\t%s' % (fname))
    # fin=open(path+fname+'.tsv','r')
    fin = pd.read_csv(path + '/groups/' + fname + '.tsv', keep_default_na=False, sep=',', encoding='utf-8')
    ...
    if not os.path.exists(path + "/stimulus-filter-%d-10-social-entropy-30-v6/" % window):
      os.makedirs(path + "/stimulus-filter-%d-10-social-entropy-30-v6/" % window)
    fout = open(path + "/stimulus-filter-%d-10-social-entropy-30-v6/" % (window) + str(fname) + '.txt', 'a')

    path_mtx_prob = path + "/stimulus-filter-%d-10-social-entropy-30-v6/group_mtx_prob/" % window
    if not os.path.exists(path_mtx_prob):
      os.makedirs(path_mtx_prob)
    # -------------------- Computing percentiles --------------------------
    """
    dperc=pd.read_csv('./data/cluster-entropy/entropy.tsv',sep='\t')
    # print(dperc.shape,fname)
    dframe=dperc[dperc.groupID==fname]
    if dframe.users.size == 0:
        size_users=0
    else:
        size_users=dframe.users.values[0]
    percentiles=np.array([0.1,0.5,0.9])
    list_percentiles=[]
    for perc in percentiles:
        size=np.int(np.floor(size_users*perc))
        list_percentiles.append(size)
    """
    list_percentiles = []
    percentiles = []
    # print(size_users,list_percentiles)
    # ---------------------------------------------------------------------
    computeStimulusDegree(fin, fout, fname, window, path_mtx_prob, \
                          list_percentiles, percentiles, fname)
...
if __name__ == '__main__':
  # path='/home/alexandre/Documentos/Whatsapp/'
  path = '/media/alexandre/Seagate Expansion Drive/Whatsapp/data/'
  ...
  # groups_idx=dgroup_info.groupID.values
  groups_idx = dgroup_info.gname.values

  lf = glob.glob('./data/stimulus-filter-1-10-social-entropy-30-v6/*.txt')
  groups_idx_done = []
  # print(lf)
  for f in lf:
    groups_idx_done.append(f[len('./data/stimulus-filter-1-10-social-entropy-30-v6/'):-4])
    # print(groups_idx[0])
    # break
  ...
  # print(groups_idx,groups_idx.size)
  # -------------------------------------------------------------------------
  # if os.path.exists('./data/groups_percentiles_v4/data_users_percentiles_ii.txt'):
  #     dt=pd.read_csv('./data/groups_percentiles_v4/data_users_percentiles_ii.txt',\
  #                    sep=',',names=['group','10º','median','90º'])
  #     # for col in dt.columns:
  #         # if col != 'group':
  #             # dt[col]=dt[col].apply(datetime.datetime.strptime,\
  #                                 # args=('%Y-%m-%d %H:%M:%S',))
  #             # dt[col]=dt[col].apply(replace_timezone)
  #     groups_idx=dt.group.unique()
  #     print('Total group\'s percentiles:',len(groups_idx))
  # else:
  #     groups_idx=np.array([])
  # -------------------------------------------------------------------------
  N_PROC = 1
  ...
  for i in range(N_PROC):
    if i < N_PROC - 1:
      ...
      # print('Index slices:\n',SIZE)
  # index_slices=[['553199096956-152026920']]
  # index_slices=[['5521992225420-145088915']]
  # index_slices=[['923450373132-152484416']]
  # index_slices=[['554599947927-151959352']]
  # index_slices=[['5511964355344-153738843','554599947927-151959352']]
  # index_slices=[['553173505906-146984627']]
  # index_slices=[['962777844696-149520649']] # teste percentiles
  # 5514998445871-150117073, 5515997619885-148565018, 553798680389-152116100,
  # 558898671406-150264285, 5521994514441-152526373, 5521988120800-150074609,
  # 557498083917-151544879, 556792542621-151196927
  # index_slices=[['962777844696-149520649']] # teste percentiles
  print('Starting...')
